using System.Web.Mvc;

namespace Teachersteams.Presentation.Areas.Teacher.Controllers
{
    public class GroupController : Controller
    {
        public ActionResult Index()
        {
            return View();
        }

        public ActionResult Details()
        {
            return View();
        }

        public ActionResult CreateGroupDialog()
        {
            return View();
        }

        public ActionResult InfoTab()
        {
            return View();
        }

        public ActionResult TeachersTab()
        {
            return View();
        }

        public ActionResult StudentsTab()
        {
            return View();
        }

        public ActionResult InviteUserDialog()
        {
            return View();
        }

        public ActionResult DeleteUserDialog()
        {
            return View();
        }

        public ActionResult EditTeacherPermissionsDialog()
        {
            return View();
        }

        public ActionResult ViewGradesDialog()
        {
            return View();
        }

        public ActionResult AssignmentsTab()
        {
            return View();
        }

        public ActionResult CreateAssignmentDialog()
        {
            return View();
        }

        public ActionResult ViewAssignmentResultsDialog()
        {
            return View();
        }

        public ActionResult GradeAssignmentResultDialog()
        {
            return View();
        }
	}
}